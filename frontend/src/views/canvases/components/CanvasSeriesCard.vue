...
const emit = defineEmits<{
  ...
  (e: 'start-rename', series: CanvasSeries): void
...
const isBuiltinSeries = computed(
  () => props.series.name === '恐龙世界' || props.series.id === 1 || props.series.sets?.some((s) => s.id === 3001)
)
...
function getSeriesSetThumbnail(series: CanvasSeries, index: number): string | null {
  const set = series.sets?.[index]
  if (!set || !set.instances || set.instances.length === 0) return null
  const inst169 = set.instances.find((i) => i.aspect_ratio === '16:9')
  return inst169 ? inst169.image_url : set.instances[0].image_url
}
</script>
...
<template>
  ...
  >
    <!-- 左上角红底白字数量胶囊 -->
    <div class="folder-count-badge">{{ series.sets?.length || 0 }}</div>
    ...
    <div class="folder-icon-wrapper">
      <div class="tianzi-grid">
        <div v-for="i in 4" :key="i" class="tianzi-cell">
          <img
            v-if="getSeriesSetThumbnail(series, i - 1)"
            :src="getSeriesSetThumbnail(series, i - 1)!"
            alt="preview"
          />
        </div>
      </div>
