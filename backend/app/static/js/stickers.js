document.addEventListener('DOMContentLoaded', () => {
    ...
    const onlyUnownedCheckbox = document.getElementById('onlyUnownedCheckbox');
    const onlyUnownedToggleLabel = document.getElementById('onlyUnownedToggleLabel');

    let currentEnergy = 0;
    let inventoryMap = new Map();
    let allSeriesConfig = [];
    let activeSeriesId = null;
    let onlyUnowned = false;

    if (onlyUnownedCheckbox) {
        onlyUnownedCheckbox.addEventListener('change', (e) => {
            onlyUnowned = e.target.checked;
            updateOnlyUnownedSwitchState();
            renderSelectedPanel();
        });
    }

    function updateOnlyUnownedSwitchState() {
        if (onlyUnownedCheckbox) onlyUnownedCheckbox.checked = onlyUnowned;
        if (onlyUnownedToggleLabel) onlyUnownedToggleLabel.classList.toggle('active', onlyUnowned);
    }
        ...
        if (stickersToRender.length === 0) {
            stickersGrid.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 50px 20px;">
                    ${onlyUnowned ? '太棒啦！该系列下的贴纸已全部拥有咯 🎨' : '该系列暂时还没有贴纸哦'}
                </div>
            `;
            return;
        }
    ...
    function renderSingleStickerCard(sticker, count, container) {
        ...
        let priceHtml = `🥚 ${sticker.exchange_price}`;
        if (isOnSale) {
