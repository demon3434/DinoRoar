...
function updateOnlyUnownedSwitchState() {
    const checkbox = document.getElementById("onlyUnownedCheckbox");
    const label = document.getElementById("onlyUnownedToggleLabel");
    if (checkbox) checkbox.checked = onlyUnowned;
    if (label) label.classList.toggle("active", onlyUnowned);
}
...
function renderCanvasesGrid() {
    ...
    if (setsToRender.length === 0) {
        grid.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 50px 20px;">
                ${onlyUnowned ? '太棒啦！该系列下的画布已全部拥有咯 🎨' : '该系列暂时还没有画布哦'}
            </div>
        `;
        return;
    }
    ...
    setsToRender.forEach(cset => {
        ...
        if (cset.is_on_sale && cset.original_price && cset.original_price > cset.exchange_price) {
            ...
            priceSpan.innerHTML = `🥚 ${cset.exchange_price}`;
        }
