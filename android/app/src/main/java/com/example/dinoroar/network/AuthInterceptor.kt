...
class AuthInterceptor @Inject constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        // 2. 捕获服务端 401 Unauthorized，标记会话过期并广播事件
        if (response.code == 401) {
            val urlPath = request.url.encodedPath
            if (!urlPath.endsWith("/api/auth/login") && !urlPath.endsWith("/api/auth/token")) {
                Log.w(TAG, "AuthInterceptor: Detected 401 Unauthorized on $urlPath! Notifying session expired...")
                securePrefs.token = null
                sessionManager.notifySessionExpired()
            }
        }

        // 3. Scan response body for lock_reset_flag = default_requested
